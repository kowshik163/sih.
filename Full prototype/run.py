...
class FRASystemRunner:
    ...
    def download_data(self, specific_datasets: List[str] = None):
        """Download required datasets"""
        self.logger.info("Downloading required datasets...")
        
        try:
            # Run the download_data.py script
            script_path = self.project_root.parent / "scripts" / "download_data.py"
            config_path = self.project_root / "configs" / "config.json"
            
            cmd = [sys.executable, str(script_path), "--config", str(config_path)]
            
            if specific_datasets:
                cmd.extend(["--datasets"] + specific_datasets)
            
            result = subprocess.run(cmd, check=True, capture_output=True, text=True)
            self.logger.info("Datasets downloaded successfully")
            self.logger.info(result.stdout)
            return True
    ...
    def run_complete_pipeline(self, data_dir: str = None, skip_downloads: bool = False):
        """Run complete pipeline from downloads to API server"""
        self.logger.info("Starting complete FRA AI Fusion pipeline...")
        
        # Step 1: Setup environment
        self.setup_environment()
        
        # Step 2: Download models and data (unless skipped)
        if not skip_downloads:
            self.logger.info("Step 2a: Downloading required models...")
            if not self.download_models():
                self.logger.error("Pipeline failed at model download stage")
                return False
            
            self.logger.info("Step 2b: Downloading required datasets...")
            if not self.download_data():
                self.logger.error("Pipeline failed at data download stage")
                return False
...
Examples:
  ...
  python run.py --download-models          # Download required models
  python run.py --download-data            # Download required datasets
  python run.py --data-pipeline           # Process raw data
  python run.py --train                   # Train fusion model
  python run.py --serve                   # Start API server
  python run.py --complete                # Run complete pipeline (with downloads)
  python run.py --complete --skip-downloads # Run pipeline without downloads
  ...
  python run.py --eval                    # Evaluate model
    ...
    parser.add_argument("--download-data", action="store_true", help="Download required datasets")
    ...
    parser.add_argument("--skip-downloads", action="store_true", help="Skip downloads in complete pipeline")
    ...
    try:
        if args.status:
            ...
            runner.download_models(args.models)
        elif args.download_data:
            runner.download_data(args.datasets)
            ...
            runner.run_complete_pipeline(args.data_dir, args.skip_downloads)
        else:
            # Default: show status
            runner.show_status()
